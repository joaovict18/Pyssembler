...
def to_bin(val, bits):
    """Return val as a two's-complement binary string with fixed width."""
    return format(val & ((1 << bits) - 1), f'0{bits}b')

def clean(line):
    """Strip inline comments and normalize punctuation to spaces."""
    line = line.split('#')[0]
    for char in ",()":
...
def main():
    ...
    with open(out_file, 'w') as f:
        if mode == '-h':
            f.write("v2.0 raw\n")
            for b in machine_code: f.write(f"{int(b, 2):08x}\n")
        else:
            f.write("\n".join(machine_code) + "\n")
